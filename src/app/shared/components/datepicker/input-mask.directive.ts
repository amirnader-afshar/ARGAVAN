...
export class InputMaskDirective implements OnInit {
  ...
  private applyMask(value): void {
    ...
    if (this.getClearValue(value).length > this.getClearValue(this.masks).length) {
    ...
    for (let i = 0; i < value.length; i++) {
      ...
      if (regexp != null) {
        ...
        newValue += current;
      } else if (this.masks[maskPosition] === current) {
        newValue += current;
      } else {
        newValue += this.masks[maskPosition];
        i--;
      }
    ...
    const nextMaskElement = this.masks[maskPosition];
  ...
  private createRegExp(position): RegExp | null {
    if (this.masks[position] == null) {
      return null;
    }

    const currentSymbol = this.masks[position].toUpperCase();
